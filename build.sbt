...
lazy val root = (project in file("."))
  .settings(
    gen := { val exitCode = Process("java",Seq("-cp", antlr_jar, "org.antlr.v4.Tool", "-o",((sourceManaged in Compile).value).getPath, "-no-listener", "-visitor", "src/main/mc/parser/MC.g4")) ! streams.value.log
    			if(exitCode != 0) sys.error(s"Antlr4 failed with exit code $exitCode") 
    			exitCode
    		  },
    grun := {
    	val exitCode = Process("java",Seq("-cp", antlr_jar, "org.antlr.v4.gui.TestRig")) ! streams.value.log
    	if(exitCode != 0) sys.error(s"Antlr4 failed with exit code $exitCode")
    }
 )
